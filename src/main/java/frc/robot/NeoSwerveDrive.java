...
public class NeoSwerveDrive {
    ...
    static final Translation2d FL_LOC = new Translation2d(NeoSwerveDef.WHEEL_BASE_WIDTH / 2,
            NeoSwerveDef.TRACK_WIDTH / 2);
    static final Translation2d FR_LOC = new Translation2d(NeoSwerveDef.WHEEL_BASE_WIDTH / 2,
            NeoSwerveDef.TRACK_WIDTH / 2);
    static final Translation2d RL_LOC = new Translation2d(NeoSwerveDef.WHEEL_BASE_WIDTH / 2,
            NeoSwerveDef.TRACK_WIDTH / 2);
    static final Translation2d RR_LOC = new Translation2d(NeoSwerveDef.WHEEL_BASE_WIDTH / 2,
            NeoSwerveDef.TRACK_WIDTH / 2);
    ...
    public static void drive() {
        xSpeed = deadzone(controller.getLeftX()) * NeoSwerveDef.SPEED_RATIO;
        ySpeed = deadzone(-controller.getLeftY()) * NeoSwerveDef.SPEED_RATIO;
        rotation = deadzone(controller.getRightX()) * NeoSwerveDef.SPEED_RATIO;

        SwerveModuleState[] states = kinematics.toSwerveModuleStates(new ChassisSpeeds(ySpeed, xSpeed, -rotation));
        // SwerveDriveKinematics.desaturateWheelSpeeds(states,
        // NeoSwerveDef.MAX_ROBOT_SPEED);
    ...
    public static void fieldOrientedDrive() {
        xSpeed = deadzone(controller.getLeftX()) * NeoSwerveDef.SPEED_RATIO * 17; // do the actual calcs
        ySpeed = deadzone(-controller.getLeftY()) * NeoSwerveDef.SPEED_RATIO * 17; // do the actual calcs
        rotation = deadzone(controller.getRightX()) * NeoSwerveDef.SPEED_RATIO * 300; // do the actual calcs

        SwerveModuleState[] states = kinematics.toSwerveModuleStates(
                ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, rotation, NeoSwerveDef.gyro.getRotation2d()));
