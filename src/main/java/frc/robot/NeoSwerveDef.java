...
public class NeoSwerveDef {
    ...
    public static final boolean RL_STEER_INVERTED = true;
    public static final boolean RR_STEER_INVERTED = true; // or true

    public static final boolean FL_ENCODER_INVERTED = false;
    public static final boolean FR_ENCODER_INVERTED = false;
    public static final boolean RL_ENCODER_INVERTED = false;
    public static final boolean RR_ENCODER_INVERTED = false;
    ...
    public static final boolean RR_DRIVE_INVERTED = true; // or true

    public static final pidValues FL_STEER_PID_VALUES = new pidValues(0.018, 0, 0);
    public static final pidValues FR_STEER_PID_VALUES = new pidValues(0.018, 0, 0);
    public static final pidValues RL_STEER_PID_VALUES = new pidValues(0.018, 0, 0);
    public static final pidValues RR_STEER_PID_VALUES = new pidValues(0.018, 0, 0);
    ...
    public static final double MAX_STEERING_SPEED_PERCENT = 0.6; // change if willing to change

    public static final double STEER_SENSOR_COEFF_TO_RAD = 2 * Math.PI / 5; // rads/volt
    public static final double STEER_SENSOR_COEFF_TE_DEG = 360.0 / 5.0; // degs/volt

    public static final double NEO_TICKS_PER_MOTOR_REV = 1; // neo returns encoder in rotations
    public static final double DRIVE_DIST_PER_WHEEL_REV = 0.1; // meters
    public static final double DRIVE_TICKS_PER_MOTOR_REV = NEO_TICKS_PER_MOTOR_REV * 8.31; // TICKS PER MOTOR REV*DRIVE
                                                                                           // GEAR RATIO
    public static final double MAX_WHEEL_SPEED = 3.62712; // m/s
    public static final double MAX_ROBOT_SPEED = 3.62712; // maybe not
    public static final double WHEEL_BASE_WIDTH = 0.45; // m
    public static final double TRACK_WIDTH = 0.44; // m

    public static final double SPEED_RATIO = 0.7;

    public static final int TIMEOUT_MS = 20;

    public static SteerMotor flSteer = new SteerMotor(4, FL_STEER_INVERTED);
    ...
    public static SwerveModule flModule = new SwerveModule(flSteer, FL_STEER_PID_VALUES, flSensor, flDrive,
            FL_DRIVE_PID_VALUES,
            FL_STEER_OFFSET, FL_ENCODER_INVERTED);

    public static SteerMotor frSteer = new SteerMotor(1, FR_STEER_INVERTED);
    public static DriveMotor frDrive = new DriveMotor(3, FR_DRIVE_INVERTED);
    public static SteerSensor frSensor = new SteerSensor(1, FR_STEER_OFFSET);
    public static SwerveModule frModule = new SwerveModule(frSteer, FR_STEER_PID_VALUES, frSensor, frDrive,
            FR_DRIVE_PID_VALUES,
            FR_STEER_OFFSET, FR_ENCODER_INVERTED);

    public static SteerMotor rlSteer = new SteerMotor(3, RL_STEER_INVERTED);
    public static DriveMotor rlDrive = new DriveMotor(2, RL_DRIVE_INVERTED);
    public static SteerSensor rlSensor = new SteerSensor(2, RL_STEER_OFFSET);
    public static SwerveModule rlModule = new SwerveModule(rlSteer, RL_STEER_PID_VALUES, rlSensor, rlDrive,
            RL_DRIVE_PID_VALUES,
            RL_STEER_OFFSET, RL_ENCODER_INVERTED);

    public static SteerMotor rrSteer = new SteerMotor(0, RR_STEER_INVERTED);
    ...
    public static SwerveModule rrModule = new SwerveModule(rrSteer, RR_STEER_PID_VALUES, rrSensor, rrDrive,
            RR_DRIVE_PID_VALUES,
            RR_STEER_OFFSET, RR_ENCODER_INVERTED);
    ...
    public static class SwerveModule { // Add PID controllers for each module
        ...
        public SwerveModule(SteerMotor sMotor, pidValues sPID, SteerSensor sSensor, DriveMotor dMotor, pidValues dPID,
                double sOffset, boolean eInverted) {
            steerMotor = sMotor;
            driveMotor = dMotor;
            drivePID = dPID;
            steerPID = sPID;
        ...
        public double getEncoderValue() {
            if (encoderInverted) {
                return Tests.invertSensor(Tests.voltageOverrun(steerSensor.getVoltage() - steerOffset));
            } else {
                return Tests.voltageOverrun(steerSensor.getVoltage() - steerOffset);
            }
        }
        ...
        public SwerveModuleState getState() {
            return new SwerveModuleState(
                    ...
                    // new Rotation2d((steerSensor.getVoltage() - steerSensor.offset) *
                    // STEER_SENSOR_COEFF_TO_RAD)); // test
                    // signs
                    new Rotation2d((steerSensor.getVoltage() - steerSensor.offset) * STEER_SENSOR_COEFF_TO_RAD));
        }

        public SwerveModuleState optimizeState(SwerveModuleState stateToOptimize, Rotation2d moduleAngle) {
            var diff = stateToOptimize.angle.minus(moduleAngle); // might be Rotation2D
        ...
        public void setAngle(double angleToSet) {
            // double encoderPosition = Tests.voltageOverrun(steerSensor.getVoltage() -
            // steerSensor.offset)
            double encoderPosition = Tests.voltageOverrun(steerSensor.getVoltage() - steerSensor.offset)
                    * STEER_SENSOR_COEFF_TE_DEG; // in
            // degs,
            // test
            // sign
            // correction
            ...
            SmartDashboard.putNumber("toSet", newAngle);
            ...
            // SmartDashboard.putNumber("steerSpeed", steerPIDController.pidGet());
        }
        ...
        public void setState(SwerveModuleState stateToSet) {
            SwerveModuleState optimizedState = optimizeState(stateToSet,
                    ...
                    // STEER_SENSOR_COEFF_TO_RAD)); // test
                    // signs
                    new Rotation2d(getEncoderValue() * STEER_SENSOR_COEFF_TO_RAD));
            // double speed =
    ...
    public static class SteerPID extends PIDController {
        ...
        public SteerPID(double kP, double kI, double kD, double mSpeed) {
            ...
            super.setTolerance(0.72); // degs
            super.enableContinuousInput(0, 360);
